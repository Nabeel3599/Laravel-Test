...
class MenuController extends BaseController
{
    ...
    public function getMenuItems()
    {
        ...
        $menu_items = Menu::where('parent_id', null)->get();

        $result =  $this->buildMenuTree($menu_items);
        return $result;
    }

    public function buildMenuTree($menu_items)
    {
        $menu = [];
        ...
        foreach ($menu_items as $item) {
            ...
            $children = Menu::where('parent_id', $item->id)->get();
            if (count($children) > 0) {
                $item_data['children'] = $this->buildMenuTree($children);
            }
